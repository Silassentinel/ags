/**
 * Obfuscator.ts - Text obfuscation utilities
 * 
 * A collection of functions for obfuscating text on the website.
...
/**
 * Obfuscator class - Encapsulates text obfuscation functionality
 * 
 * This class provides methods for transforming text in various ways,
 * primarily used to create chaos effects in conjunction with ChaosMode.
 */
export class Obfuscator {
  ...
  /**
   * Integrate with chaos mode when it exists
   * This function is called internally by the obfuscator module
   * Side effects: Sets up timers and event listeners
   */
  public static initWithChaosMode(): void {
    ...
    document.addEventListener('DOMContentLoaded', () => {
      ...
      const checkChaosState = (): void => {
        try {
          ...
          if (clickCount >= 25) { // Ultra chaos threshold
            ...
            if (Math.random() < 0.3) { // 30% chance of obfuscation
              ...
              this.obfuscateElements(selector, { method });
          ...
          console.error('Error checking chaos state:', error);
        }
      };
      ...
      document.addEventListener('click', () => {
        try {
          ...
          if (clickCount >= 25 && Math.random() < 0.5) {
            const selector = 'p, h1, h2, h3, h4, h5, h6';
            this.obfuscateElements(selector, { method: 'zalgo' });
          ...
          console.error('Error in click handler for obfuscation:', error);
        }
      });
    });
  }
...
// Export static method references for backward compatibility
export const getRandomMethod = Obfuscator.getRandomMethod.bind(Obfuscator);
export const obfuscate = Obfuscator.obfuscate.bind(Obfuscator);
export const obfuscateElements = Obfuscator.obfuscateElements.bind(Obfuscator);
export const restoreElements = Obfuscator.restoreElements.bind(Obfuscator);
export const cyclicObfuscation = Obfuscator.cyclicObfuscation.bind(Obfuscator);

/**
 * Initialize the obfuscator with ChaosMode integration
 * This is the public API for integrating obfuscator with chaos mode
 */
export const initObfuscatorWithChaosMode = (): void => {
  if (typeof window === 'undefined') return;
  Obfuscator.initWithChaosMode();
};

// Initialize the obfuscator with ChaosMode integration
// Only runs if in browser environment
