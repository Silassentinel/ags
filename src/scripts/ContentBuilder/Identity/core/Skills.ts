/**
 * Core skills definitions and exports for the application
 */

/**
 * Represents a generic skill with name and level
 */
...
/**
 * Represents a technical skill (programming languages, tools, etc.)
 */
export type TechSkillType = SkillType & {
    category: 'frontend' | 'backend' | 'devops' | 'database' | 'other';
    years: number;
};

/**
 * Represents a soft skill (communication, leadership, etc.)
 */
export type SoftSkillType = SkillType & {
    description: string;
};

/**
 * Compare skills by level for sorting
 */
export const compareSkillsByLevel = (a: SkillType, b: SkillType): number => {
